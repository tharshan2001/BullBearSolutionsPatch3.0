...
const SubscriptionCard = ({ subscription }) => {
  ...
  // Helpers
  const handleClick = () => navigate(`/products/detailed/${product.slug}`);
  ...
  const getDaysRemaining = () => {
    ...
    return diffDays > 0 ? `${diffDays} day${diffDays !== 1 ? 's' : ''} left` : 'Expires today';
  };

  // Status styling with icons
  const getStatusStyle = () => {
    const base = "px-2.5 py-1 rounded-md text-xs font-medium flex items-center gap-1.5";
    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 space-y-1.5">
          <div className="flex justify-between items-start">
            <h3 className="text-white font-medium truncate pr-2">{product.Title}</h3>
      ...
      <div className="mt-4 pt-3 border-t border-slate-700/50 space-y-2.5">
        <div className="flex justify-between text-sm">
          ...
          <span className={`font-medium ${autoRenew ? 'text-teal-400' : 'text-amber-400'}`}>
        ...
        {expiresAt && (
          <div className="flex justify-between text-sm">
            ...
            <span className={`font-medium ${status === 'active' ? 'text-amber-400' : 'text-rose-400'}`}>
